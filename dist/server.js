    ...
    try {
        ...
        try {
            ...
            audioStream.on('error', (e) => {
                console.error('[ytdl] error:', e?.message || e);
            });
        ...
        try {
            ...
            if (e?.status)
                console.error('OpenAI status:', e.status);
            if (e?.response?.data)
                console.error('OpenAI error body:', e.response.data);
            throw e;
        }
        // Normalize response
        const result = typeof response.text === 'string' ? { text: response.text } : response;
        res.json({
            ok: true,
            model,
            url,
